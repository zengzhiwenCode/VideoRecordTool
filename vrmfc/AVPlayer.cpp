...
CAVPlayer::CAVPlayer(void)
	...
	m_hWnd(nullptr),
	m_pfnPlaying(nullptr),
	m_pfnPosChanged(nullptr),
	m_pfnEndReached(nullptr)
{
	//m_data.player = this;
...
bool CAVPlayer::Play(const std::string &strPath)
{
	...
	if (m) {
		...
		if (m_pVLC_Player) {
			...
			libvlc_event_attach(vlc_evt_man, libvlc_MediaPlayerPlaying, &OnVLC_Event, data);
			libvlc_event_attach(vlc_evt_man, libvlc_MediaPlayerPositionChanged, &OnVLC_Event, data);
			libvlc_event_attach(vlc_evt_man, libvlc_MediaPlayerEndReached, &OnVLC_Event, data);
...
void CAVPlayer::Stop()
{
	if (m_pVLC_Player) {
		...
		void * data = this;
		libvlc_event_manager_t *vlc_evt_man = libvlc_media_player_event_manager(m_pVLC_Player);
		libvlc_event_detach(vlc_evt_man, libvlc_MediaPlayerEndReached, &OnVLC_Event, data);
		libvlc_event_detach(vlc_evt_man, libvlc_MediaPlayerPositionChanged, &OnVLC_Event, data);
		libvlc_event_detach(vlc_evt_man, libvlc_MediaPlayerEndReached, &OnVLC_Event, data);
...
void OnVLC_Event(const struct libvlc_event_t *event, void *data)
{
	//UserData* user = (UserData*)data; if (!user) return;
	CAVPlayer *player = (CAVPlayer *)data;

	pfnCallback pfn = nullptr;
	...
	case libvlc_MediaPlayerPlaying:
		pfn = player->m_pfnPlaying;
		break;
	case libvlc_MediaPlayerPositionChanged:
		pfn = player->m_pfnPosChanged;
		break;
	case libvlc_MediaPlayerEndReached:
		pfn = player->m_pfnEndReached;
		break;
	default:
		break;
	}
	...
	if (pfn) {
		pfn(player);  // 此回调函数还可以传入其他参数，除了data外，还有event的各种信息（如event->u.media_player_position_changed.new_position）等等，请自行扩展。
	}
}
