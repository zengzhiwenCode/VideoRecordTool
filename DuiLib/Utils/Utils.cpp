...
{
	ASSERT(!::IsBadStringPtrW(lpsz, UINT_MAX) || lpsz == NULL);
	m_szBuffer[0] = '\0';
	Assign(lpsz, nLen);
}
...
{
	if (lpStr) {
		ASSERT(!::IsBadStringPtr(lpStr, UINT_MAX));
		Assign(lpStr);
	} else {
		Empty();
	}
...
{
	if (lpStr) {
		ASSERT(!::IsBadStringPtrA(lpStr, UINT_MAX));
...
{
	ASSERT(!::IsBadStringPtr(pstrSub, UINT_MAX));
	ASSERT(iPos >= 0 && iPos <= GetLength());
	if (iPos != 0 && (iPos < 0 || iPos > GetLength())) return -1;
